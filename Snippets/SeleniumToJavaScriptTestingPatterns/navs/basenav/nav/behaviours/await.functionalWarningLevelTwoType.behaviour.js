...
export default class AwaitBehaviourCanCycleThroughAlerts extends AwaitableAssertableActionBehaviour
{
  ...
  PerformYields(arr,rte,o)
  {
    ...
    {
      //mock data in initial await so one of each type alert is waiting in the cards
        //will allow you to open, go to next, go to next, then close
      yield 'SPY / wait for & Verify dropdown open with warning level two type'
    }

    function* PerformAwaitAction()
    {
      let actions = ['CALL DRIVER','ACKNOWLEDGE','CANCEL RIDE']
    ...
    {
      yield 'click on hitbox or close'
    };
