...
export default class AwaitBehaviourCanCycleThroughAlerts extends AwaitableAssertableActionBehaviour
{
  ...
  PerformYields(arr,rte,o)
  {
    ...
    {
      //mock data so one of each type alert is waiting in the cards
        //will allow you to open, go to next, go to next, then close
      yield 'SPY / wait for & Verify dropdown open with failed type'
    }

    function* PerformAwaitAction()
    {
      const PerformAwaitBE = new BehaviourExecutor(),
            actions = ['REBOOK','REVIEW TRIP','CANCEL RIDE']
      PerformAwaitBE.Execute([
